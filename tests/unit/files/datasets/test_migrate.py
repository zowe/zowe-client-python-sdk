...
class TestCreateClass(TestCase):
    ...
    def test_recall_migrated_data_set(self, mock_send_request):
        """Test recalling migrated data set sends a request"""
        mock_send_request.return_value = mock.Mock(headers={"Content-Type": "application/json"}, status_code=200)

        Files(self.test_profile).recall_migrated_data_set("dataset_name")
        mock_send_request.assert_called_once()

    def test_recall_migrated_data_set_parameterized(self):
